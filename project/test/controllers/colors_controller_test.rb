...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference("Color.count") do
      post colors_url, params: { color: { code: @color.code, name: @color.name, opacity: @color.opacity } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { code: @color.code, name: @color.name, opacity: @color.opacity } }
