...
1.   实现以产品为中心的部分：`产品`、`颜色`、`类型`、`大小` 和 `款式`，不难发现这与 `Blog4` 项目种 `recipe` 和 `category` 的关系如出一辙；
2.   实现以用户为中心的部分：`用户` 和 `收获地址`，利用 devise 插件可以快速构建用户管理的基础设施，从而避免重复造轮子，并区分出 `user` 和 `admin` $2$ 种 `role`；
...
又有 Rails 单元测试框架使用起来不是那么方便（比如与用户登录，用户身份鉴权结合起来的时候），故没有编写单元测试，而是每完成一个小模块，就在浏览器端测试验证，边开发边测试。
...
*   当收获地址关联到订单项（无论订单项的生命周期）时，不允许删除：
...
总的来说，`对角巷 Online ` 是一次非常不错的前后端不分离的敏捷开发实践。在写同期的前后端分离的数据库大作业时，仅 $6$ 个实体我就写了非常久:sob:， 而 `对角巷 Online `  虽然有 $11$ 个实体，但可以相对很多的达成需求。